<template>
  <div class="feed" ref="feedRef">
    ...
    <div v-if="needLogin && !loading" class="empty">请先登录后查看朋友（互相关注）的视频</div>
    <div v-else-if="!items.length && !loading" class="empty">暂无朋友（互相关注）的视频</div>
  </div>
</template>
...
async function load(page=1) {
  ...
  try {
    // 获取我关注的人，并筛选互相关注（朋友）
    const ures = await api.followingQuery({ page, order: 'comprehensive' })
    const users = Array.isArray(ures?.results) ? ures.results : []
    // 互相关注
    const mutuals = users.filter(u => !!(u && (u.is_mutual === true || u.is_mutual === 1)))
    // 控制请求量：最多取前 6 个朋友，每个取 4 条视频
    const promises = mutuals.slice(0, 6).map(u => api.videosList({ userId: u.id, pageSize: 4, order: 'latest' }).catch(() => ({ items: [] })))
    ...
    if (aborted) return
    hasNext.value = Boolean(ures?.has_next || (mutuals.length >= 6))
    curPage.value = Number(ures?.page || page || 1)
  } catch (e) {
    if (Number(e?.status) === 401) { needLogin.value = true; items.value = []; displaySrc.value = []; try { loaded.clear() } catch(_) { void 0 } }
